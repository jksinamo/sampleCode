...
![](edgesExample.png) weight is defined as the number of edges between the two nodes
![](nodesExample.png) depth is defined as the recursive count from the seed node set to the node. If there are duplicates of integers in the list, it means that the node was found multiple times in that particular depth. The length of this list can be converted as the weight of that node (or the number of in-links of that node) depending on your needs. 
